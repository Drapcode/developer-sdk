# DrapcodeApis  Documentation

The `DrapcodeApis` class provides methods for interacting with the Drapcode API. This documentation outlines how to use the class and its available methods.
...
```typescript
import { DrapcodeApis } from 'path/to/DrapcodeApis';

const drapcodeApi = new DrapcodeApis(project_seo_name: string, xApiKey?: string, authorization?: string, environment?: string);


DrapcodeApis Class Documentation
The DrapcodeApis class facilitates interactions with the Drapcode API. This documentation provides a comprehensive guide on how to use the class and its methods effectively.

Instantiation
To utilize the DrapcodeApis class, instantiate it with the following parameters:
...
xApiKey (optional): Your API key for authentication, if required.
...
Example  const drapcodeApi = new DrapcodeApis("singhaniya-computer-7138");
...
Example const items = await drapcodeApi.getAllItems("users");
...
Example await drapcodeApi.createItem("users", {
    "name": "Pk",
    "age": 23
});
...
Example const filteredItems = await drapcodeApi.getItemsWithFilter("users", "15263");
...
collectionName: The name of the collection to retrieve items from.
filterUuid: The UUID of the filter to apply.
Example const itemCount = await drapcodeApi.getItemsCountWithFilter("users", "15263");
...
Example const item = await drapcodeApi.getItemWithUuid("users", "3487-383");
...
Example await drapcodeApi.updateItemWithUuid("users", "3487-383", {"name": "Drapcode"});
...
Example  drapcodeApi.deleteItemWithUuid("users", "3487-383");
...
Example await drapcodeApi.sendEmail("345-678", "support@drapcode.com");
